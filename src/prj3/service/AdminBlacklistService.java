...
import prj3.dao.AdminBlacklistDAO;
...
public class AdminBlacklistService {
	...
	public List<BlacklistViewDomain> searchBlacklistList(SearchVO sVO){
		List<BlacklistViewDomain> list = null;
		
		AdminBlacklistDAO abDAO = AdminBlacklistDAO.getInstance();
		
		try {
			list = abDAO.selectBlacklistList(sVO);
	...
	public int totalCount(SearchVO sVO) {
		...
		AdminBlacklistDAO abDAO = AdminBlacklistDAO.getInstance();
		try {
			totalCnt = abDAO.selectTotalCount(sVO);
		} catch (SQLException e) {
			e.printStackTrace();
		} // end catch
	...
	public List<WarningTypeDomain> searchWarningType(){
		List<WarningTypeDomain> list = null;
		
		AdminBlacklistDAO abDAO = AdminBlacklistDAO.getInstance();
		
		try {
			list = abDAO.selectWarningType();
		} catch (SQLException e) {
			e.printStackTrace();
		}//end catch
	...
	public JSONObject addBlacklist(InsertBLacklistVO ibVO) {
		...
		try {
			flag = abDAO.insertBlacklist(ibVO);
		} catch (SQLException e) {
			e.printStackTrace();
		}//end catch
